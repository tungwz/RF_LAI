    ...
    for i in reg:
        ...
        if np.any(lc==13):
            ...
            if len(valid_indices[0])>=10:
                ...
                # var  = np.array((lai_reg, htop_reg, tmax_reg, tmin_reg, prec_reg, sw_reg, vp_reg, lat_reg, lon_reg))
                # data = pd.DataFrame(var.T, columns=['lai','htop','tmax','tmin','pre','sw','vp', 'lat', 'lon'])
...
def load_predict_data(reg, iyear, imon):
    ...
    # lat_ = np.repeat(np.array(lat[:,np.newaxis]),1200,axis=1)
    # lon_ = np.repeat(np.array(lon[np.newaxis,:]),1200,axis=0)
...
def process_region(ireg):
    # reg_lc, reg_lat, reg_lon, reg_htop, reg_tmax, reg_tmin, reg_prec, reg_vp, reg_sw, reg_lat_, reg_lon_ = load_predict_data(ireg, iyear, i)
    reg_lc, reg_lat, reg_lon, reg_htop, reg_tmax, reg_tmin, reg_prec, reg_vp, reg_sw, reg_vpd = load_predict_data(ireg, iyear, i)

    # reg_var  = np.array((reg_htop, reg_tmax, reg_tmin, reg_prec, reg_sw, reg_vp, reg_lat_, reg_lon_)).T
...
for iyear in range(2000,2021,1):
    ...
    for i in range(12):
        ...
        # print(" Train r2_score is %f, RMSE is %f, MAE is %f" %(r2_score(np.exp(y_train), np.exp(train_pre)),sqrt(mean_squared_error(np.exp(y_train), np.exp(train_pre))),mean_absolute_error(np.exp(y_train), np.exp(train_pre))))#.astype('str')))
        # print(" Test r2_score is %f, RMSE is %f, MAE is %f" %(r2_score(np.exp(y_test), np.exp(test_pre)), sqrt(mean_squared_error(np.exp(y_test), np.exp(test_pre))),mean_absolute_error(np.exp(y_test), np.exp(test_pre))))#.astype('str')))
