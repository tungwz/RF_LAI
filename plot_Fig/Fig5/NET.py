...
# plt.subplots_adjust(left=0.2, right=0.8)#, top=0.95, bottom=0.1)  # 调整 left 和 right 的值
for i in range(len(info['site'])+2):
	...
	if (i==2):
		plt.ylabel('LAI (m$^{2}$/m$^{2}$)', font2)
		# ax.set_ylabel('LAI (m$^{2}$/m$^{2}$)', font2, labelpad=200)   #  $m^3s^{-1}$
...
fig.suptitle('Evergreen Needleleaf Tree', fontproperties='DejaVu Sans',fontsize=120)
# fig.tight_layout()
# plt.show()
fig.subplots_adjust(top=0.95)
plt.savefig('./NET.pdf', format='pdf', bbox_inches='tight', pad_inches=0.15, dpi=600)
