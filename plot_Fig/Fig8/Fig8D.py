...
def draw_distribution_histogram(nums, nums1, nums2, path, month):
  ...
  if month==9 or month==10 or month==11 or month==12:
    ...
    plt.xticks(x, x_label)#, fontsize=14)
  else:
    x_label= [' ', ' ', ' ', ' ', ' ']#[' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ']
...
lai_nc = nc4.Dataset('../../plot_Fig/nc_data/Global_GLASS_LAI_0.5deg_2020.nc')
...
sns.kdeplot(np.random.normal(size=100), shade=True, color='steelblue', label='GLASS')  # 示例数据
sns.kdeplot(np.random.normal(size=100), shade=True, color='red', label='GLASS/%Tree')  # 示例数据
sns.kdeplot(np.random.normal(size=100), shade=True, color='purple', label='GLASS/%(Tree+Grass)')  # 示例数据
plt.ylim(0, 1000)
plt.legend(loc='upper right', prop={'size': 15})
plt.axis('off')  # 不显示坐标轴
plt.tight_layout()
plt.savefig('./legend2.pdf', format='pdf', dpi=600)  # 保存图例
plt.close()  # 关闭图例图形
